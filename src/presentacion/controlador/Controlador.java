	...
	public void accion(int evento, Object objeto) {
		...
		switch(evento)
		{
			case EventosTurno.ANADIR_TURNO:{ // Comienzo de las acciones que realiza el módulo TURNO
				...
				try{
					Integer resultado = saTurno.altaTurno(turno);
					if(resultado != -1 && resultado != -2){
						...
						mensaje = "No pueden existir dos turnos con el mismo Nombre.";
						gui.actualizar(EventosTurno.ANADIR_TURNO_KO, mensaje);
					}
					else if(resultado == -2){
						mensaje = "Los valores introducidos para el turno no son válidos.";
						gui.actualizar(EventosTurno.ANADIR_TURNO_KO, mensaje);
					}
				} catch(Exception e){
			...
			case EventosTurno.ELIMINAR_TURNO:{
				...
				SATurno saTurno = sa.generaSATurno();
				try{
					Integer resultado = saTurno.bajaTurno(id);
					if(resultado == -1) {
						mensaje = "No se ha encontrado ningún turno con el ID proporcionado.";
						gui.actualizar(EventosTurno.ELIMINAR_TURNO_KO, mensaje);
					}
					else if(resultado == -2) {
						mensaje = "El turno proporcionado aun tiene empleados vinculados.";
						gui.actualizar(EventosTurno.ELIMINAR_TURNO_KO, mensaje);
					}
					else{
			...
			/*case EventosFactura.FACTURAS_PAGADAS_POR_CLIENTE: {
				...
				try{
					...
					if(listaFacturasCliente != null){
						gui.actualizar(EventosFactura.FACTURAS_CLIENTE_OK, listaFacturasCliente);
					}
					else{
						mensaje = "El cliente seleccionado no ha efectuado el pago de ninguna factura.";
						gui.actualizar(EventosFactura.FACTURAS_CLIENTE_KO, mensaje);
					}
				}catch(Exception e){
					mensaje = "No se ha podido conectar con la base de datos.";
					gui.actualizar(EventosFactura.FACTURAS_CLIENTE_KO, mensaje);
			...
			case EventosFactura.APLICAR_DESCUENTO:{
				...
				SAFactura saFactura = sa.generaSAFactura();
				try{
					Double descuento = saFactura.aplicarDescuento(factura);
					if(descuento != -1) {
						gui.actualizar(EventosFactura.APLICAR_DESCUENTO_OK, descuento);
					}
					else{
						mensaje = "El descuento no puede ser un valor negativo.";
			...
			case EventosFactura.FACTURAS_PAGADAS_POR_CLIENTE:{
				...
				SAFactura saFactura = sa.generaSAFactura(); 
				try{
					LinkedList<TFactura> listaFacturasCliente = saFactura.listarFacturasPorCliente(idCliente);
					if(listaFacturasCliente != null){
						gui.actualizar(EventosFactura.FACTURAS_CLIENTE_OK, listaFacturasCliente);
					}
					else{
			...
			case EventosEmpleado.REGISTRAR_EMPLEADO:{ //Comienzo de las acciones que realiza el módulo EMPLEADO
				...
				try{
					...
					if(resultado != -1 && resultado != -2){
						...
						mensaje = "No se puede vincular un empleado con un turno no existente.";
						gui.actualizar(EventosEmpleado.REGISTRAR_EMPLEADO_KO, mensaje);
					}
					else{
						mensaje = "No pueden existir dos empleados con el mismo DNI.";
						gui.actualizar(EventosEmpleado.REGISTRAR_EMPLEADO_KO, mensaje);
					}
				} catch(Exception e){
					mensaje = "No se ha podido conectar con la base de datos.";
			...
			case EventosEmpleado.LISTAR_EMPLEADOS_POR_TURNO:{
				Integer idTurno = (Integer) objeto;
				SAEmpleado saEmpleado = sa.generaSAEmpleado();
				try{
					LinkedList<TEmpleado> listaEmpleados = saEmpleado.listarEmpleadosPorTurno(idTurno);
					if(listaEmpleados != null){
						gui.actualizar(EventosEmpleado.LISTAR_EMPLEADOS_POR_TURNO_OK, listaEmpleados);
					}
					else{
						mensaje = "No se han encontrado empleados registrados que estén relacionados con la información propuesta.";
						gui.actualizar(EventosEmpleado.LISTAR_EMPLEADOS_POR_TURNO_KO, mensaje);
					}
				}catch(Exception e){
					mensaje = "No se ha podido conectar con la base de datos.";
					gui.actualizar(EventosEmpleado.LISTAR_EMPLEADOS_POR_TURNO_KO, mensaje);
