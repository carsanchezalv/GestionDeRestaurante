...
public class EmpleadoGUIImpl extends JPanel implements EmpleadoGUI {
		...
		enviarBtn.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){
				...
				if ((nombre.length() > 0 && !nombre.equals(" ")) && (DNI.length() > 0 && !DNI.equals(" ")) && (IDTurno.length() > 0 && !IDTurno.equals(" "))) {
					TEmpleado empleado = new TEmpleado(nombre, DNI, true, Integer.valueOf(IDTurno));
	...
	public void empleadosTurnoPanel() {
		...
		buscarBtn.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){
				String ID = IDField.getText();	
				if (ID.length() > 0 && !ID.equals(" ")) {
					SingletonControlador.getInstancia().accion(EventosEmpleado.LISTAR_EMPLEADOS_POR_TURNO, Integer.valueOf(ID));
				} else {
	...
	public void actualizar(int evento, Object datos) {
		...
		switch (evento){
			...
			case EventosEmpleado.LISTAR_EMPLEADOS_OK:
				listaEmpleados = (List<TEmpleado>) datos;
			...
			case EventosEmpleado.LISTAR_EMPLEADOS_KO:
				...
				break;
			case EventosEmpleado.LISTAR_EMPLEADOS_POR_TURNO_OK:
				listaEmpleados = (List<TEmpleado>) datos;

				for (TEmpleado e : listaEmpleados) {
					empleadosModel.addRow(new Object[]{e.getId(), e.getNombre(), e.getDni(), e.getIdTurno().toString(), Boolean.valueOf(e.getActivo())});
				}
				
				empleadosMainPanelCL.show(empleadosMainPanel, "EMPLEADOS");
				System.out.println("Listar Empleados OK");
				break;
			case EventosEmpleado.LISTAR_EMPLEADOS_POR_TURNO_KO:
				mensaje = (String) datos;
